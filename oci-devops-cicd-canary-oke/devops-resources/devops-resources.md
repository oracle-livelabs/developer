...
In this lab you will be taking a close look of major OCI  resources ,which were provisioned in the previous lab of this series.The same included OCI Devops , OKE , VCN and other related resources mentioned below.
...
4. In the samepage you should see a project named as `DevOpsCanaryOKE_devopsproject_<Unique Key>`,click on it.
...
8. Go back to the project overview page ,click on the Latest Build Pipeline named `Function-Python-Buildpipeline`.It will open up the build pipeline and show the different stages. 

   ![oci_devops_canary-build-pipeline](images/oci-devops-buildpipeline-1.png)

9. Build pipeline containes four stages 

   - A manage build stage to build the python application.
   - A deliver artifact stage to push the image to OCI Container registry repo.
...
10. Click on the `3 dots` and view details for each of the stages.
   ...
   Optional : You can refere more here about [OCI Build stages](https://docs.oracle.com/en-us/iaas/Content/devops/using/managing_build_pipelines.htm)
...
12. Deployment pipeline will have 4 stages
   ...
   - A Canary OKE Deployment to deploy the change to canary namespace.
   ...
   - A Deploy OKE to deploy the change to production namespace.
...
13. Switch back to Devops project overview and from `Latest environments` select the environment named as `oke_environment_<ID>`.
...
15. Switch back to `Devops Project overview` and refer the devops artifacts created under `Latest artifacts`

   ![oci-devops-artifacts](images/oci-devops-artifacts.png)

16 . We are using two artifacts ,where one is for `Docker image` and another one for  `Deployment - Kubernetes manifest` . Docker image artifact is used to push the docker image to container registry repo during `Deliver artifact` stage with in the build piepline. The `Kubernetes manifest` is for the deployment to OKE using devops deployment pipeline.
...
3. Under the OKE cluster , user option `Access Cluster` and Follow `Cloud Shell Access` method.

   ![oci-oke-access-1](images/oci-oke-access-1.png)

   ![oci-oke-access-2.png](images/oci-oke-access-2.png)

4. This will open the `OCI Cloud shell` and where in you need to copy and past the command in the access instruction.

   ![oci-oke-access-over-cs](images/oci-oke-access-over-cs.png)

5. Run the command `kubectl cluster-info ` on Cloud shell  and validate the `Kubernetes API public endpoint` address with the information from OCI Console view..

   ![oci-kubectl-cluster-ip](images/oci-kubectl-cluster-ip.png)
   
   ![oci-oke-cluster-ui](images/oci-oke-cluster-ui.png)

6. Run command `kubectl get ns` to validate that there are three namespaces crated as 
...
As the OKE cluster doesn't have an ingress controller by default, an NGINX ingress controller is explicitly installed when we created the infrastrecture via ORM jobs.
...
2. Click on `Build Pipelines` and click `oci_devops_canary-build-pipeline`.With in the pipeline , click on `Start manual run` button on the top right corner.

   ![](images/oci-build-start-manually.png)

3. Use button `Start manual run` and start the exeuction .
...
5. Wait for all the stages to complete , This would take approximatly 5 to 7 minutes.
...
8. Once its complete the first two stages (Deploy to Canary namespace and Traffic shift) ,it will be pending in the  `Approval STage.` . As its the very first execution we dont see any Canary effect here as there is nothing deployed in to the production namespace to compare with .
...
10. Provide a comment and approve it .
...
1. In order to validate ,we will be using `OCI Cloud shell` and fetch the information from `OKE` namespace. Open cloud shell from the `OCI Console` top right corner.

   ![oci-cs](images/oci-cs.png)


2. Run the command `kubectl cluster-info ` on Cloud shell  and validate the `Kubernetes API public endpoint` address with the information from OCI Console view..
...
3. Fetch the informaton from OKE namespace `nscanaryprd` ,via running command `kubectl get all,ingress -n nscanaryprd` on cloud shell.
...
4. From the above get the ingress `ADDRESS`
   ...
   Optional : You can run command `kubectl get all,ingress -n nscanarystage` and fetch the information from canary namespace which will be exact that of prod namespace ,as its the very first deployment.
