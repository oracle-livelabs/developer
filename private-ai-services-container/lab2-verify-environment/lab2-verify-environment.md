...
Sam is ready to test the first architecture, but starts with a basic engineering rule: confirm that the services can see one another before diagnosing code. The team uses the JupyterLab terminal as a checkpoint for the private network.

![Sam validates that JupyterLab, Private AI, Oracle AI Database, and ORDS are reachable](./images/story-verify-runtime.png)

Sam asks you to take the first checkpoint. From a **JupyterLab Terminal**, you trace the service names across the private network, then call the Private AI Services Container health endpoint. A successful response tells the team that the service is alive; a model listing shows which inference tools are available before they choose one.

You also check the Oracle AI Database and ORDS paths. These checks matter because the later embedding labs depend on all of the services working together. If the network and endpoints are healthy now, any later problem is more likely to be in the notebook code or the model workflow rather than the environment.
