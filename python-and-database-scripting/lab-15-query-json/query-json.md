...
2. Check that the record has been added to the table by counting the number of rows in TEST\_JSON\_TABLE.
...
Delete the non-JSON row(s) from TEST\_JSON\_TABLE.

In Code Editor, append the following lines to the *query\_json.py* file

````
<copy>
cur.execute('delete from TEST_JSON_TABLE where id=2')
...
* **Last Updated By/Date** - Veronica Dumitriu, Product Management, May 2023
