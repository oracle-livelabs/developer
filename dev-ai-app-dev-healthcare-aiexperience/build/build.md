...
Here you’ll create an end‑to‑end prior‑authorization system using Oracle AI Database and OCI Generative AI. After connecting to clinical datasets, you’ll analyze patient records and leverage an LLM to recommend approval, pend, or denial - complete with evidence citations. By applying Python skills from prior labs, you’ll assemble a fully integrated, AI‑driven healthcare review portal.

This lab uses some of the basic coding samples you created in the previous lab.
...
2. Navigate to `db_setup_HCS_script_2.sql` under the `dbinit` folder. Here is where you can see all the tables that will be created for the healthcare industry. 

    ![HCS-Table](./images/hcs-table.png " ")
...
## Task 6: Create a function to generate recommendations for the patients
...
1. Copy the following code and run:
    ...
    ```python
        <copy>
        # Chunk & store AI recommendation, then SHOW the chunks
...
1. We embed the recommendation chunks and our policy text for retrieval. Copy the following code into a new cell block:
    ...
    ```python
        <copy>
        # Create embeddings for MEDICAL_CHUNK rows
        auth_id = authorization_id  # from previous tasks
...
* **Contributors** - Linda Foinding, Uma Kumar
* **Last Updated By/Date** - Hanna Rakhsha, December 2025