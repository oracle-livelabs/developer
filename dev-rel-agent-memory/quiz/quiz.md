...
Q: What is the main purpose of the CityOps Copilot built in this notebook?
* To help inspectors reason from current observations, prior findings, and accumulated memory
- To replace the Oracle database with an external vector database
- To generate synthetic city infrastructure assets
- To train a new LLM from inspection reports
> The copilot combines structured asset data, vector search, scoped memory, and LLM reasoning so inspectors can use both current and historical context.

Q: What is the difference between domain data and memory in this lab?
...
Q: Why does the notebook use Oracle’s in-database ONNX embedding model?
* To compute embeddings inside Oracle without sending text to an external embedding service
...
> The embedder adapter exposes Oracle’s in-database embedding capability to the SDK, keeping embedding generation close to the data.
