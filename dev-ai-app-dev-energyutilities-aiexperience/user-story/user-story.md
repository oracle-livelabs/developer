...
2. Select **Energy** under Industry and **Grid Engineer Advisor** under Role. Enter in a username and click **Login**.
...
4. Select the **Navigate To Supplier Decisions** button.
    ...
    After navigating to the decisions page, the AI evaluation runs in the background. Energy documents containing feeder information such as host capacity, voltage impacts, and fault margins are stored in Oracle AI Database and modeled with JSON Duality Views. These documents are vectorized and searched using AI Vector Search to identify semantically relevant prior requests and compliance criteria. The results are passed to OCI Generative AI, which uses retrieval-augmented generation (RAG) to generate a natural-language explanation, formatted for both internal review and external communication. 
...
7. Set the final status to **Approved**, then click **Confirm Decision** to complete the process.

    The status has been updated to 'Approved' and saved to the supplier profile.
...
12. Expand **View Approved Suppliers**. We can see that Aisha Morgan has been removed from the Pending Suppliers list and has been added to the Approved Suppliers list.
...
2. Opening Nina Wright’s profile displays her supplier history details. Within a few seconds, the AI automatically generates a suggested action. In this case, the system evaluates the profile and assigns it to “high risk” with explanations outlining key risk factors.
...
4. Expand **Interactive Graph: DER Request, Risk & Decision** to view the graph.
    ...
    On the decision page, the Grid Engineer can use **Interactive Graph: DER Request, Risk & Decision** to explore relationships in supplier authorization scenarios. Built with **Oracle Graph**, this feature visually maps decisions and highlights important features of the request like the feeder analysis and compliance rules.
...
6. The authorization status is set to **Denied**. Click the **Confirm Decision** button.

    The authorization status has been updated to 'Denied' and saved to the supplier profile.
...
9. Display the message the supplier would see by opening the downloaded PDF.
...
11. Expand **View Denied Supplier**. You will see that Nina Wright has been moved from the **Pending Suppliers** list to the **Denied Suppliers** list.
...
Lastly, let’s explore how the system uses JSON Duality Views to handle profile updates. In this task, you will edit a supplier's details. In this example, the supplier was asked to submit a compliance explanation after doing an inspection.
...
2. We will upload a document to update the compliance explanation. On the supplier's details page, click the **Upload Document** button. The PDF file has been loaded. Then click the **Process PDF** button
...
4. The supplier profile has been updated.

    ![Customer Profile Updated](./images/profile-updated.png =50%x*)

5. Refresh the page and note the updated analysis. The uploaded compliance document indicated "Fault current exceeds safe limits," resulting in a new DER recommendation — ChargePoint Express (EV Charger) now replaces Enphase IQ8 (Microinverter).

    ![Customer Profile Updated](./images/updated-der.png =50%x*)
...
In conclusion our Grid Interconnection Approval App was able to leverage Oracle AI Database technologies such as **AI Vector Search, Property Graph and JSON Duality Views** to:

✅ Automate profile evaluations

✅ Provide AI-driven energy recommendations by using a RAG model powered by a Oracle AI Database's AI Vector Search and OCI Generative AI service

✅ Enable seamless profile updates with JSON Duality Views

✅ And empower grid engineer advisor with actionable insights through Operational Property Graphs 
...
## Acknowledgements
* **Authors** - Uma Kumar
* **Contributors** - Francis Regalado, Hanna Rakhsha, Ley Sylvester, Patrick Mccormick
